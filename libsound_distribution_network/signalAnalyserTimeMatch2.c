...
extern char printSignalDetect;
extern char printAnalyseEventFreq;
extern char printAnalyseSignal;
extern char printAnalyseSignalMatch;
extern char printAnalyseEventPeak;
...
char fi_checkFreq2(struct FrequencyInfo *_this, struct EventInfo *_event, float _minMatchFreqYDistance)
{
	if (_event == NULL)
	{
		_this->endReason = 11;
...
void fi_addTime(struct FrequencyInfo *_this, const struct FrequencyMatch *_fm)
{
	...
	if (_this->maxAmplitudeIdx < 0 || _this->maxAmplitudeIdx >= times_size || _fm->frequency->amplitude > matchTimes[_this->maxAmplitudeIdx].frequency->amplitude)
	{
...
char fis_isSignalNeedDiscard(struct FrequencyInfoSearcher *_this, struct FrequencyInfo *_pfi, int _maxFreqYDistance)
{
...
{
	char freqTruncate = false;
	char signalTruncate = false;	
	char checkingFreqHasSignal = false;
	char erased = false;
	int i, c, x, f, fc;
		...
		for(i = 0; i < (int)vector_size(&_this->checkingFreq); (!erased)?i++:i)
		{
			...
			if (!fi_checkFreq2(pfi, _ei, _maxFreqYDistance))
			{
				...
				if (fi_howLongTime(pfi) >= freqCountLong )
				{
					...
					int ii;
					char hasMoreLongSignal = fis_isSignalNeedDiscard(_this, pfi, _maxFreqYDistance);
...
void fis_truncateSignal(struct FrequencyInfoSearcher *_this, float _maxFreqYDistance)
{
	...
	for(i = 0, c = vector_size(&_this->checkingFreq); i < c; i ++)
	{
		...
		{
			char hasMoreLongSignal = fis_isSignalNeedDiscard(_this, fi, _maxFreqYDistance);
			if(!hasMoreLongSignal)
			{
...
char bs_hasBlock(struct BlockSearcher *_this, struct FrequencyInfo **_blockHead, struct FrequencyInfo **_blockTail, char *_isEndBlock)
{
...
void bs_onFFTEvent(struct BlockSearcher *_this, struct EventInfo *_ei)
{
...

	{
...
#endif
		{
			...
			else
			{
				...
				{
					char s2_1OK = (fi_timesFirst(&_this->blockFIs[1])->event->timeIdx - fi_timesFirst(&_this->blockFIs[2])->event->timeIdx) >= ((RS_CORRECT_BLOCK_SIZE+RS_CORRECT_SIZE) * preciseMatchCount);
				...
				else if (_this->blockCount > 2)
				{
					...
					char s2_1OK = s2_1_Len > ((RS_CORRECT_BLOCK_SIZE+RS_CORRECT_SIZE) * preciseMatchCount);
					assert(_this->unsedBlockIdx >= 1);
					if(!s2_1OK)
					{
						removedFIIdx = bs_removeWeakestBlock(_this, 0, 2, &removedFI);
		...
		&& ((fi_timesFirst(&_this->blockFIs[_this->okBlockIdx])->event->timeIdx - fi_timesFirst(&_this->blockFIs[_this->okBlockIdx+1])->event->timeIdx) < ((RS_CORRECT_BLOCK_SIZE+RS_CORRECT_SIZE-1) * preciseMatchCount)))
	{
		...
		char lastBlockInvalid = _this->blockFIs[_this->okBlockIdx].blockSignalCount < 2 
...
void fats_onFFTAmpsRange(struct FreqAmpsEventStat *_this, struct FFTAmpsRange *_amps, float _minPeak2Avg, float _minStatPeak2Avg, float _maxFreqYDistance)
{
	...
	hasValidAmplitude = maxFreqAmplitude->amplitude / ((_this->longTermAvgAmplitude > avgAmplitude)?avgAmplitude:_this->longTermAvgAmplitude) > _minPeak2Avg;
...
void initCodeY(struct SignalAnalyser *_this, char _force)
{
	int i;
...
char sa_analyFFTAmplitude2FrequencyInfo(struct SignalAnalyser *_this, struct EventInfo *_ei)
{
	return true;
}
...
int getFrequencyInfoEventDeviation(int _freqY, tidx _eventStart, tidx _eventEnd, float _matchCount, int _signalEventIdxOff, char _printLog)
{
	...
	printEventsIdxOff("freq %d(%d, %d) match (%d, %d) deviation %d of eventoff %d\n", _freqY
		, _eventStart, _eventEnd, fiRangeStart, fiRangeEnd, absMinus, _signalEventIdxOff);
	return absMinus;
}
	...
	for (i = _startEvent; i <= _maxEndEvent+1; i ++)
	{
		...
		else
		{
			if (sameCount+1 >= MIN_SIGNAL_BIG_MATCH)
			{
				...
				matchSignalEvents(_this, siganlMaxEvent->p1.frequencyY, siganlMaxEvent->timeIdx, &signalStartEvent, &signalEndEvent);
				for (j = 0; j < searchMinRangeCount; j ++)
				{
					eventDeviation = getFrequencyInfoEventDeviation(siganlMaxEvent->p1.frequencyY, signalStartEvent->timeIdx, signalEndEvent->timeIdx, _matchCount, j, false);
			...
			{
				char curr2prepreEqual;
				if (prepreEvent == NULL || prepreEvent->p1.frequencyY == 0 || currEvent == NULL || currEvent->p1.frequencyY == 0)
				{
					curr2prepreEqual = false;
				}
				else
				{
					curr2prepreEqual = abs(currEvent->p1.frequencyY - prepreEvent->p1.frequencyY) <= _this->maxFreqYDistance;
				}
				if (curr2prepreEqual)
				{
...
int sa_analyValidSignals3(struct SignalAnalyser *_this, int _eventStart, int _eventEnd, char _isPreciseEnd, char _isFullBlock)
{
	...
	char isFullBlock = _isFullBlock;
	if(_this->freqRanges == NULL)_this->freqRanges = (struct FIMatch *)mymalloc(sizeof(struct FIMatch) * DEFAULT_CODE_FREQUENCY_SIZE);
...

	{
		...
		for (i = 0; i < blockSignalCount; i ++)
		{
				...
				{					
					...
					for (prex = 0; prex < MAX_RANGE_IDX_COUNT; prex ++)
					{
						...
						if (preRangeSignal->idxes[prex] == targetIdx)
						{
							...
							if (preMatchs[preSignalIdxInPreMatches.idxes[prex]].sumQuality/preMatchs[preSignalIdxInPreMatches.idxes[prex]].sumAmplitude > (rangeIdxEnd-rangeIdxStart+1) * 0.75)
							{
								...
								{
									memmove(preRangeSignal->idxes+prex, preRangeSignal->idxes+prex+1, MAX_RANGE_IDX_COUNT-prex-1);
			...
			if(targetIdx < 0)
			{
				...
				currSignalIdx.idx2 = curMatchs[((curMatchs[matchIdx+1].rangeIdx==preTargetIdx)?(matchIdx+2):(matchIdx+1))].rangeIdx;
