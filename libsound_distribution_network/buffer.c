...
inline struct BufferData *bd_getNullBuffer(void)
{
...
//判断buffer data是否为空
inline char bd_isNULL(struct BufferData * _this)
{
...
struct Buffer *b_init2(struct Buffer *_this, int bufferCount, int bufferSize, char _createData)
{
	int i2 = 0;
	...
	for (i2 = 0; i2 < _this->mBufferCount; ++i2)
	{
		...
		char r = cq_try_enqueue(&_this->mProducerQueue, &tempData);
		assert(r);
...
void b_reset(struct Buffer *_this)
{
	...
	for (i = 0; i < size; ++i)
	{
		...
		{
			char r = cq_try_enqueue(&_this->mProducerQueue, &data);
			assert(r);
		}
	}
#ifndef COND_NO_MUTEX
...
{
	struct BufferData *data = NULL;
	char r = cq_try_dequeue(&_this->mProducerQueue, &data);
	if(r)
	{
		cnd_broadcast(&_this->cond);
		return data;
...
//将data的数据 入队到生产者队列（循环缓冲队列）
char b_putEmpty(struct Buffer *_this, struct BufferData *data)
{
...
int putFullIdx = 0;
char b_putFull(struct Buffer *_this, struct BufferData *data)
{
	if (0 == data)return false;
	if (!bd_isNULL(data))
	{
...
char b_putImpl(struct Buffer *_this, struct BufferData *data, TBlockingQueue *queue)
{
	if (0 != queue && 0 != data)
	{
#ifndef COND_NO_MUTEX
