...
char grl_onAfterECC(struct MyRecognitionListener *this_, float _soundTime, int _recogStatus, int *_indexs, int _count)
{
	struct GeneralRecognitionListener *_this = (struct GeneralRecognitionListener *)this_;
...
void *vr_createVoiceRecognizer2(enum DecoderPriority _decoderPriority, int _sampleRate)
{
	...
	printf("voiceRecog bufferSize = %d\n",bufferSize);
...
VOICERECOGNIZEDLL_API int vr_getVer(void)
{
	return MYVERSION;
}
