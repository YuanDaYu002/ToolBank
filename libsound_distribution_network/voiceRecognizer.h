...
struct RecognitionListener {
	...
	char (*onStopRecognition)(struct RecognitionListener *_this, float _time, int _recogStatus, int *_indexs, int _count);
#ifdef SEARCH_SIMILAR_SIGNAL
	char (*onStopRecognition2)(struct RecognitionListener *_this, float _time, int _recogStatus, struct TimeRangeSignal*_indexs, int _count, 
...
struct MyRecognitionListener
{
	...
	char (*onAfterECC)(struct MyRecognitionListener *_this, float _soundTime, int _recogStatus, int *_indexs, int _count);
};
...
char mrl_onStopRecognition(struct RecognitionListener *_this, float _soundTime, int _recogStatus, int *_indexs, int _count);
#ifdef SEARCH_SIMILAR_SIGNAL
char mrl_onStopRecognition2(struct RecognitionListener *this_, float _soundTime, int _recogStatus, struct TimeRangeSignal* _indexs, int _count
	, struct SignalBlock *_blocks, int _blockCount);
#endif
char mrl_decode(struct MyRecognitionListener *_this, int *indexs, int signalLen);
...
struct BufferDataWriter *bdw_init(struct BufferDataWriter *_this, struct BufferSource *_bufferSource, int _channel, int _bits, int _bufferSize, int _overlap, char _fillFloatBuffer);
...
struct MaybeSignal
{
	char discoveryFinished, recognizeFinished;
	int signalIdx;
	int totalDetectCount, validDetectCount;
	TPos startPos, endPos;
	unsigned long startTime, endTime, recognizeStartTime;
	char (*isDiscoveryFinished)(struct MaybeSignal *_this);
};

struct MaybeSignal *ms_init(struct MaybeSignal *_this);
void ms_reset(struct MaybeSignal *_this);
void ms_onDetect(struct MaybeSignal *_this, char _hasSignal);
char ms_isDiscoveryFinished(struct MaybeSignal *_this);
...
char mmsq_isDiscoveryFinished(struct MaybeSignal *_this);
...
struct FreqSignalDetector
{
	...
	char normalAmplitudeInited;
	char preHasSignal, prepreSignal, preprepreSignal, prepreprepreSignal;
	char hasNewSignal;
};
...
void fsd_detect(struct FreqSignalDetector *_this, struct VoiceEvent *_event, struct kiss_fft_out *_fftOut, char _ignore);
...
struct PreprocessVoiceProcessor
{
	...
	volatile char realProcessing;
	mtx_t m;
	cnd_t cond;
	thrd_t realProcessThread;
	volatile char realProcessNULL;
	volatile char realProcessThreadQuited;
...
struct VoiceRecognizer
{
	...
	volatile char paused;	//是否暂停
	char stopped;		//是否退出（停止）
};
...
void vrr_setStopped(struct VoiceRecognizer *_this, char _stopped);
...
char vrr_isStopped(struct VoiceRecognizer *_this);
...
struct AudioConverter *getConverter(int _channel, int _bits, char _signed, char _bigEndian);
