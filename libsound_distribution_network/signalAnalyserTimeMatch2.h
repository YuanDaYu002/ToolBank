...
inline char trs_maybeError(struct TimeRangeSignal *_this)
{
	return _this->flags;
}
inline void trs_setMaybeError(struct TimeRangeSignal *_this, char _val)
{
	_this->flags = _val;
}
...
char fi_checkFreq(struct FrequencyInfo *_this, struct EventInfo *_event, int _minMatchFreqYDistance);
