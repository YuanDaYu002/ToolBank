...
char cq_peek2(struct CircularQueue *_this, void ** _elem)
{
	char r = false;
	mtx_lock(&_this->m);
...
char cq_peekBotton1(struct CircularQueue *_this, void **_elem)
{
	char r = false;
	mtx_lock(&_this->m);
...
char cq_try_push(struct CircularQueue *_this, void *_ele)
{
	char r = false;
	mtx_lock(&_this->m);
...
//出队成功返回true，失败（无数据了）返回false
char cq_try_pop(struct CircularQueue *_this, void *_pelem)
{
	char r = false;
	mtx_lock(&_this->m);
...
inline char cq_empty(struct CircularQueue *_this){return (_this->m_iSize > 0)? true : false;};
...
inline char cq_try_enqueue(struct CircularQueue *_this, void * element)
{
...
inline char cq_try_dequeue(struct CircularQueue *_this, void *_pelem)
{
