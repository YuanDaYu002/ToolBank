...
char mrl_onAfterECC(struct MyRecognitionListener *_this, float _soundTime, int _recogStatus, int *_indexs, int _count)
{
...
void fsd_detect(struct FreqSignalDetector *_this, struct VoiceEvent *_event, struct kiss_fft_out *_fftOut, char _ignore)
{
	...
	{
		char hasSignal = (hasSignal_ == DR_Yes);
	...
	if (!(_event->data == NULL || bd_isNULL(_event->data)))
	{
		...
		for (i = 0; i < _this->sampleStepSize; i ++)
		{
			...
			if (_this->longTermCount >= VD_LONGTERM_LONG_MIN)
			{
				...
				char isPeak = (recentAvg2LongAvg > _this->minRecentAvg2LongtermAvg);
...
char mmsq_isDiscoveryFinished(struct MaybeSignal *this_)
{
...
void vrr_run(struct VoiceRecognizer *_this)
{
	...
	vrr_setStopped(_this, false);//设置 VoiceRecognizer 开始运行
	b_reset(&_this->buffer);	//重置 buffer
	while (true)
	{
		//printf("vrr_run while(1)...\n");
		char paused = false;
		data = b_getFull(&_this->buffer); //从消费者队列获取一个元素的数据
		...
		if(!bd_isNULL(data))//buffer data 不为空则将数据入队到生产者队列
		{
			b_putEmpty(&_this->buffer, data);
		}
		else break;			//为空则退出循环，结束检测
		
	}

	printf("break ! vrr_setStopped !!!!********************************************\n");
	vrr_setStopped(_this, true);//设置 VoiceRecognizer 停止运行
