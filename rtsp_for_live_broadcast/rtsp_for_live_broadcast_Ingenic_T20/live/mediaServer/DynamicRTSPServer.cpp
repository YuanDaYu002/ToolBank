...
static ServerMediaSession* createNewSMS(UsageEnvironment& env,
  ...
  if (strcmp(extension, ".aac") == 0) {
    ...
    NEW_SMS("H.264 Video");
    OutPacketBuffer::maxSize = 600000;//100000; // allow for some possibly large H.264 frames
    sms->addSubsession(H264VideoFileServerMediaSubsession::createNew(env, fileName, reuseSource));
  } else if (strcmp(extension, ".265") == 0) {
    // Assumed to be a H.265 Video Elementary Stream file:
    NEW_SMS("H.265 Video");
    OutPacketBuffer::maxSize = 600000;//100000; // allow for some possibly large H.265 frames
...
//#define INTERLEAVE_ADUS 1
    ...
    OutPacketBuffer::maxSize = 600000;//300000;

    NEW_SMS("DV Video");
    sms->addSubsession(DVVideoFileServerMediaSubsession::createNew(env, fileName, reuseSource));
  } else if (strcmp(extension, ".mkv") == 0 || strcmp(extension, ".webm") == 0) {
    // Assumed to be a Matroska file (note that WebM ('.webm') files are also Matroska files)
    OutPacketBuffer::maxSize = 600000;//100000; // allow for some possibly large VP8 or VP9 frames
