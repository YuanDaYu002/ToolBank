...
// buffer[0~4]->int32 大端字节序转换成小端字节序
...
// prase moov: buffer->moov  成功：返1 失败：返0
int ParseMoov(OUT moov_t* moov, IN unsigned char* buffer, IN unsigned int size)
{
	...
	while (buffer < buffer_start + size)
	{
		buffer = ReadHeader(buffer, &leaf_atom);//读8字节的box头，寻找 cmov  mvhd       	trak
		PrintInfo(&leaf_atom);
		if (CompareType(&leaf_atom, "cmov"))//好像没有这样的box类型
		{
			return 0;
		}
		...
		buffer = ToEnd(&leaf_atom);//跳转到该box的结尾处（下一个box的开始处）
	}
...
// write stco offsets: 就是把所有chunk的偏移量都加offse;
...
bool mp4_segment(OUT mp4Buffer& outMp4, mp4Buffer& inMp4, float iStartTime, float iEndTime)
{
	...
	while (pData < pEnd)//遍历完所有的box 寻找 moov box 和 mdat box 
	{
		atom_t tempAtom;
		pData = ReadHeader(pData, &tempAtom);//解析box头部，前8字节都是box 的头部， boxsize 和 boxtype
		...
		pData += tempAtom.size_ - ATOM_PREAMBLE_SIZE;//每次比较后往后跳转当前box的长度（ReadHeader（） 多跳转了 ATOM_PREAMBLE_SIZE，这里要减回来）
	}
