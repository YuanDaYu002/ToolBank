    ...
    public: \
        ...
        void SetValue(uint##isize##_t value, uint32_t index = 0) { \
            if (m_readOnly) { \
               ...
               /* throw new PlatformException(msg.str().c_str(), EACCES, __FILE__, __LINE__, __FUNCTION__); USE_LiteOS_SYSTEM */ \
            } \
...
#if 1 // USE_LiteOS_SYSTEM : 0   otherwise :1 ，此处先不更改
	void SetValue(const char* value, uint32_t index = 0);
#else
	int32_t SetValue(const char* value, uint32_t index = 0);
#endif
