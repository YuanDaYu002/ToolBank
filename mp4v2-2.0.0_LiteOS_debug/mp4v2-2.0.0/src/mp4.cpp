...
///////////////////////////////////////////////////////////////////////////////
    ...
    MP4FileHandle MP4Create (const char* fileName,
                             uint32_t  flags)
    {
		/*******debug try...throw ...catch*****************************/
#if 0
		try
		{
			printf("try printf aaa\n");
			printf("try printf bbb\n");
			
			throw new Exception("Invalid parameters",  __FILE__, __LINE__, __FUNCTION__);
		 
			
			printf("try printf ccc\n");
                               ...
                               uint32_t supportedBrandsCount)
    {
        ...
        MP4File* pFile = ConstructMP4File();
		printf("ConstructMP4File back!\n");
        if (!pFile)
            return MP4_INVALID_FILE_HANDLE;
		

#if 0
		printf("MP4CreateEx into try!\n");
	try {
			printf("MP4CreateEx into ASSERT!\n");
            ASSERT(pFile);
			printf("MP4CreateEx into Create!\n");
			...
			printf("MP4CreateEx at Create back!\n");
            return (MP4FileHandle)pFile;
       }
        catch( Exception* x ) {
            mp4v2::impl::log.errorf(*x);
		...
		printf("MP4CreateEx at 003!\n");
        if (pFile)
            delete pFile;
		printf("MP4CreateEx at return!\n");
        return MP4_INVALID_FILE_HANDLE;
    }
...
// returns true if the track is a media track encrypted according to ismacryp
    ...
    bool MP4AddIPodUUID (MP4FileHandle hFile, MP4TrackId trackId)
    {
		...
		#if defined (USE_LiteOS_SYSTEM)
				track = ((MP4File*)hFile)->GetTrack(trackId);  //GetTrack函数已经针对LiteOS已进行修复
				if(NULL == track)
					return false;
        ...
        try
        {
            ipod_uuid = new IPodUUIDAtom(*(MP4File*)hFile);//针对 LiteOS 此处先不做更改，不认为容易抛出错误,内部已经屏蔽throw
        }
        catch( std::bad_alloc ) {
        ...
        try
        {
            ASSERT(avc1);
            ASSERT(ipod_uuid);
            avc1->AddChildAtom(ipod_uuid);//针对 LiteOS 此处先不做更改，不认为容易抛出错误,内部没有继续追踪
            return true;
        }
        catch( Exception* x ) {
            delete ipod_uuid;
