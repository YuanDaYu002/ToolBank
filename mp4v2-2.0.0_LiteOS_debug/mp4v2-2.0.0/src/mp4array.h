    ...
    public: \
        ...
        void Insert(type newElement, MP4ArrayIndex newIndex) { \
            if (newIndex > m_numElements) { \
                /*  throw new PlatformException("illegal array index", ERANGE, __FILE__, __LINE__, __FUNCTION__); USE_LiteOS_SYSTEM */ \
            } \
        ...
        void Delete(MP4ArrayIndex index) { \
            if (!ValidIndex(index)) { \
              ...
              /*  throw new PlatformException(msg.str().c_str(), ERANGE, __FILE__, __LINE__, __FUNCTION__); USE_LiteOS_SYSTEM */\
            } \
            m_numElements--; \
