...
#if 0 //defined ( USE_LiteOS_SYSTEM ) //使用兼容 LiteOS 的函数实现,先不更改 
int32_t MP4StringProperty::SetValue(const char* value, uint32_t index)
{
    if (m_readOnly)//只读 
	{
		return -1;
...
void MP4StringProperty::SetValue(const char* value, uint32_t index)
{
    if (m_readOnly) {//只读
		...
		#else
			cerr << "property " << m_name << "is read-only" << endl;
		#endif
