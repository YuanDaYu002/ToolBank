                      ...
                      uint32_t    supportedBrandsCount )
{
    ...
    m_createFlags = flags;
	/*解Liteos 不兼容的bug,****************************/
	//自己实现一套文件操作接口，提供给open函数
	#if 0
	MP4FileProvider Provider_LiteOS;
	Provider_LiteOS.open = LiteOS_open;
	Provider_LiteOS.close = LiteOS_close;
	Provider_LiteOS.read = LiteOS_read;
	Provider_LiteOS.seek = LiteOS_seek;
	Provider_LiteOS.write = LiteOS_write;
	#endif
	/*************************************************/
   // Open( fileName, File::MODE_CREATE, NULL );//BUG，进去后挂了
	Open( fileName, File::MODE_CREATE, NULL );
	
	printf("into MP4Atom::CreateAtom!\n");
    // generate a skeletal atom tree
    m_pRootAtom = MP4Atom::CreateAtom(*this, NULL, NULL);
	printf("into Generate !\n");
    m_pRootAtom->Generate();

	printf("into Generate back!\n");
    if (add_ftyp != 0) {
		printf("into MakeFtypAtom!\n");
        MakeFtypAtom(majorBrand, minorVersion,
                     supportedBrands, supportedBrandsCount);
    }
	printf("into CacheProperties!\n");
    CacheProperties();

	printf("into InsertChildAtom!\n");
    // create mdat, and insert it after ftyp, and before moov
    (void)InsertChildAtom(m_pRootAtom, "mdat",
                          add_ftyp != 0 ? 1 : 0);

	printf("into   m_pRootAtom->BeginWrite!\n");
    // start writing
    m_pRootAtom->BeginWrite();
    if (add_iods != 0) {
		printf("into   AddChildAtom!\n");
...
void MP4File::Optimize( const char* srcFileName, const char* dstFileName )
{
    ...
    try {
        ...
        Open( srcFileName, File::MODE_READ, NULL );//暂时未处理
        ReadFromFile();
        CacheProperties(); // of moov atom
...
void MP4File::Open( const char* name, File::Mode mode, const MP4FileProvider* provider )
{
	...
	printf("into new File! m_file = %x\n",m_file);
    m_file = new File( name, mode, provider ? new io::CustomFileProvider( *provider ) : NULL );
	
	printf("into m_file->open!  m_file = %x\n",m_file);
	
	if( m_file->open() ) 
	{
		printf("into m_file->open  002!\n");
        ostringstream msg;
	...
	printf("into switch( mode ) mode = %d\n",mode);
    switch( mode ) {
        case File::MODE_READ:
...
{
	#if defined (USE_LiteOS_SYSTEM)
		uint16_t ret = FindTrackIndex(trackId);
		if(ret == (uint16_t)-1)//trackID不存在
			return NULL;
		else
			return m_pTracks[ret];
...
int32_t MP4File::FindStringProperty(const char* name,
                                 MP4Property** ppProperty, uint32_t* pIndex)
{
    if (!FindProperty(name, ppProperty, pIndex)) {
        ostringstream msg;
...
const char* MP4File::GetStringProperty(const char* name)
{
	...
	#if defined (USE_LiteOS_SYSTEM)
	 	int32_t ret = FindStringProperty(name, &pProperty, &index);
		if(ret < 0)
			return NULL;
	#else
...
{
	printf("into MP4File::AddTrack ! file:%s  line:%d\n",__FILE__,__LINE__);
    ProtectWriteOperation(__FILE__, __LINE__, __FUNCTION__);
	printf("into MP4File::AddTrack 01! file:%s  line:%d\n",__FILE__,__LINE__);
    // create and add new trak atom
    MP4Atom* pTrakAtom = AddChildAtom("moov", "trak");
    ASSERT(pTrakAtom);
	printf("into MP4File::AddTrack 02! file:%s  line:%d\n",__FILE__,__LINE__);

	
    // allocate a new track id
    MP4TrackId trackId = AllocTrackId();  ///BUG  occur
	printf("into MP4File::AddTrack 03! file:%s	line:%d\n",__FILE__,__LINE__);

    m_trakIds.Add(trackId);
	printf("into MP4File::AddTrack 04! file:%s  line:%d\n",__FILE__,__LINE__);
	
    // set track id
    MP4Integer32Property* pInteger32Property = NULL;
    (void)pTrakAtom->FindProperty("trak.tkhd.trackId",
                                  (MP4Property**)&pInteger32Property);
	printf("into MP4File::AddTrack 05! file:%s  line:%d\n",__FILE__,__LINE__);
    ASSERT(pInteger32Property);
    pInteger32Property->SetValue(trackId);
	printf("into MP4File::AddTrack 06! file:%s  line:%d\n",__FILE__,__LINE__);

    // set track type
    const char* normType = MP4NormalizeTrackType(type);
	printf("into MP4File::AddTrack 07! file:%s	line:%d\n",__FILE__,__LINE__);
    ...
    uint8_t audioType)
{
	...
	printf("into AddAudioTrack!\n");
    MP4TrackId trackId = AddTrack(MP4_AUDIO_TRACK_TYPE, timeScale);
	printf("into AddAudioTrack  01!\n");
    AddTrackToOd(trackId);
	printf("into AddAudioTrack  02!\n");
    SetTrackFloatProperty(trackId, "tkhd.volume", 1.0);
	printf("into AddAudioTrack  03!\n");
    (void)InsertChildAtom(MakeTrackName(trackId, "mdia.minf"), "smhd", 0);
	printf("into AddAudioTrack  04!\n");

    (void)AddChildAtom(MakeTrackName(trackId, "mdia.minf.stbl.stsd"), "mp4a");
	printf("into AddAudioTrack  05!\n");

    AddDescendantAtoms(MakeTrackName(trackId, NULL), "udta.name");
	printf("into AddAudioTrack  06!\n");
...
{
	printf(" MP4File::AllocTrackId!\n");
    MP4TrackId trackId = GetIntegerProperty("moov.mvhd.nextTrackId");
	printf("into MP4File::AllocTrackId 01! trackId = %d  file:%s  line:%d\n",trackId,__FILE__,__LINE__);
    if (trackId <= 0xFFFF) 
	{
		#if defined (USE_LiteOS_SYSTEM)
		
			printf("into MP4File::AllocTrackId 02! file:%s  line:%d\n",__FILE__,__LINE__);
            uint16_t ret = FindTrackIndex(trackId);
			if(ret == (uint16_t)-1)
			{
		...
		#else
			//LiteOS 不支持try...throw ...catch这种异常抛出结构，在调用throw的时候会导致系统崩溃
	        // check that nextTrackid is correct
	        try 
			{
				printf("into MP4File::AllocTrackId 02! file:%s  line:%d\n",__FILE__,__LINE__);
	            (void)FindTrackIndex(trackId);   ///BUG 
	            // ERROR, this trackId is in use
	            printf("into MP4File::AllocTrackId 03! file:%s  line:%d\n",__FILE__,__LINE__);
	        }
	        catch (Exception* x) 
			{
				printf("into MP4File::AllocTrackId 04! file:%s  line:%d\n",__FILE__,__LINE__);
	...
	printf("into MP4File::AllocTrackId 06! file:%s  line:%d\n",__FILE__,__LINE__);

	#if defined (USE_LiteOS_SYSTEM)
		//LiteOS not support  "throw"
	#else
	...
	printf("into return MP4_INVALID_TRACK_ID  file:%s  line:%d\n",__FILE__,__LINE__);
...
/*
	传入的 trackId 不存在，直接返回(uint16_t)-1
	存在，则返回可用的下标(uint16_t)i
*/
uint16_t MP4File::FindTrackIndex(MP4TrackId trackId)
{
	printf("into FindTrackIndex ! m_pTracks.Size() = %ld\n",m_pTracks.Size());
    ...
    for (uint32_t i = 0; i < m_pTracks.Size() && i <= 0xFFFF; i++) 
	{
		...
		{
			printf("  return (uint16_t)i \n ");
            return (uint16_t)i;
        }
    }
	printf("into FindTrackIndex  001!\n");

    ostringstream msg;
    msg << "Track id " << trackId << " doesn't exist";
	printf("into FindTrackIndex  002!\n");

	#if defined (USE_LiteOS_SYSTEM)
		//使用返回值“(uint16_t)-1”来表示当前 trackId 可用，屏蔽掉throw语句
	#else
    	throw new Exception(msg.str(),__FILE__, __LINE__, __FUNCTION__);
	#endif
	printf("into FindTrackIndex  003!\n");
...
void MP4File::AppendSessionSdp(const char* sdpFragment)
{
    ...
    try {
		...
		printf("LiteOS Abnormal detected!\n");
        delete x;
        SetSessionSdp(sdpFragment);
        return;
    }
